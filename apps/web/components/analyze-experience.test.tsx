...
function fillRequiredInputs() {
  ...
  fireEvent.change(screen.getByLabelText("Source text"), {
    target: { value: "A company announced a new fulfilment centre in India." },
  });
}

describe("AnalyzeExperience", () => {
  it("invokes the action boundary and renders its typed result", async () => {
    ...
    expect(
      JSON.stringify(vi.mocked(productClient.trackEvent).mock.calls),
    ).not.toContain("Example Retail Ltd");
  });
  ...
  it("keeps generation successful when analytics is unavailable", async () => {
    ...
    expect(
      screen
        .getByRole("button", { name: "Analyze opportunity" })
  ...
  it("discloses that fixture results do not analyze custom input", () => {
    ...
    expect(
      screen.getByText(
        /typed sample result and does not analyze custom input/i,
      ),
    ).not.toBeNull();
  });
  ...
  it("preserves inputs and maps recoverable backend errors", async () => {
    const productClient = client({
      generateOpportunity: vi.fn(async () =>
        Promise.reject({ code: "RESEARCH_FAILED" }),
      ),
    });
    render(<AnalyzeExperience client={productClient} />);
